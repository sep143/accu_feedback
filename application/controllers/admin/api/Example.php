...
//            $data['branding_logo'] = ($branding->b_home_logo != '')?base_url().'uploads/'.$dat['restaurant_id'].'/'.$branding->b_home_logo: base_url().'image/app_logo.png';
    ...
    public function survey_post(){
        ...
        // $surveyData['restaurant_id'] = '1';

        // $surveyData['survey_id'] = '207';

        // $surveyData['device_info'] = 'SM-N910C';

        // $surveyData['device_imei'] = '1e9894f1375e37df';

        // $surveyData['answer_date'] = date('Y-m-d H:i:s');

        // $responses['answer_json'] = json_decode('{"response":[{"type":"1","sequence_no":1,"value":2.0},{"type":"3","sequence_no":2,"value":4},{"type":"5","sequence_no":3,"value":[{"type":"checkbox","name":"chk1","value":true},{"type":"number","name":"Number 1","value":"12345312542"}]},{"type":"5","sequence_no":4,"value":[{"type":"text","name":"enter name","value":"D"},{"type":"email","name":"email","value":"a@b.c"},{"type":"number","name":"number","value":"21345213542"}]},{"type":"5","sequence_no":5,"value":[{"type":"text","name":"Enter your full name.","value":"Dilip"},{"type":"email","name":"Enter your email address","value":"dilip9@gmail.com"},{"type":"number","name":"Enter your mobile number.","value":"9673645123"}]}]}', true);

        // $surveyData['answer_json'] = '{"response":[{"type":"1","sequence_no":1,"value":2.0},{"type":"3","sequence_no":2,"value":4},{"type":"5","sequence_no":3,"value":[{"type":"checkbox","name":"chk1","value":true},{"type":"number","name":"Number 1","value":"12345312542"}]},{"type":"5","sequence_no":4,"value":[{"type":"text","name":"enter name","value":"D"},{"type":"email","name":"email","value":"a@b.c"},{"type":"number","name":"number","value":"21345213542"}]},{"type":"5","sequence_no":5,"value":[{"type":"text","name":"Enter your full name.","value":"Dilip"},{"type":"email","name":"Enter your email address","value":"dilip9@gmail.com"},{"type":"number","name":"Enter your mobile number.","value":"9673645123"}]}]}';

        // $surveyData['waiter_code'] = '830830';
        ...
        if (!empty($surveyData['restaurant_id']) && !empty($surveyData['survey_id'])) {
            ...
            $restaurant_id = $surveyData['restaurant_id'];
        ...
        if($check_trigger){
            ...
            foreach ($check_trigger as $trigger_count =>$trigger_data){
             ...
             foreach ($data['trigger_condition']['condition'] as $tri_count=>$tri_n):
                ...
                }elseif($trigger_condition['question_seq'] == "Total" && $trigger_condition['type'] == "Total Score") {
           ...
           $tri_data[] = $data;
            ...
            foreach ($data['trigger_score'] as $coutn=> $n):
                    ...
                    $check_score = $data['responses_value'][$coutn];

                    $check_condition = htmlspecialchars_decode($data['trigger_condition1'][$coutn]);

                    // $check_value = $data['responses_value'][$coutn];
                    $check_value = $data['trigger_score'][$coutn];
            ...
            if($data['trigger_rule'] == 'OR'){
                ...
                if($common == true){
                    ...
                    $notification=array(
                        ...
                        'device'=>$surveyData['device_info'],

                        // 'device_time'=>$this->post('answer_date'),
                        'device_time'=>$surveyData['answer_date'],
                        ...
                        'responses'=>$surveyData['answer_json']

                    );

                    // $this->response([
                    //         'trigger_rule_calling' => "Call",
                    ...
                    //         // 'notification_added' => $add_notification
                    //     ]);
                    ...
                    //$this->load->library('email');
                     ...
                     $config = Array(
                        ...
                        $this->email->from("contact@accufeedback.com", "Feedback Form");
            ...
            if ($insert){
                ...
                $data['is_info_required'] = $this->Survey_model->get_survey_by_id($surveyData['survey_id']);

                if($data['is_info_required']->customer_info == "YES") {
                    
                    $responseAnswer = json_decode($surveyData['answer_json'],true);

                    $getLastResponse = end($responseAnswer['response']);

                    if(count($getLastResponse['value'])==3) {
                        
                        $data['customer_info'] = array( 
                            'restaurant_id' => $surveyData['restaurant_id'],
                            'survey_id'     => $surveyData['survey_id'], 
                            'name'          => $getLastResponse['value'][0]['value'],
                            'email'         => $getLastResponse['value'][1]['value'],
                            'mobile_no'     => $getLastResponse['value'][2]['value']
                        );    
                    }

                    $this->Customer_model->customer_info_add($data['customer_info']);
                    
                }

                $this->thanks_sms_send_mobile($data['customer_info']['mobile_no'],$data['customer_info']['name']);
     ...
     * SMS Send
     */ 
    public function thanks_sms_send_mobile($mobile=null,$customer_name=null){
        // $mobile = "9636452007";
        $key = "291528AwcfPDpoOt5d6638a9"; //msg91.com/api/ >> For use Of VegHut
        // $mobile="9636452007";
        $sender_id = "ACFDBK";
        // $msg = "Thank you for visiting our restaurant, ".$customer_name.". It was a pleasure to have you over, and we hope that we'll see you again, soon! :)";

        $msg = "Thank you for visiting our restaurant, ".$customer_name.". if you like the food then please rate 5 star on google. https://tinyurl.com/y56jvuoz";
        ...
        /*
         *  this bulksmsgateway.in
         */
        $url = "https://api.msg91.com/api/sendhttp.php?mobiles=".urlencode($mobile)."&authkey=".$key."&route=4&sender=".$sender_id."&message=".urlencode($msg)."&country=91";
