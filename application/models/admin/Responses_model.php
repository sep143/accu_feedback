...
class Responses_model extends CI_Model{
    ...
    function get_all_responses($id){
        $sql="select * from survey_answer1 where restaurant_id='".$id."' ORDER BY answer_date DESC";
        $data=  $this->db->query($sql,$id);
        return $data->result();
    }
    function get_response($id, $admin_id){
        $sql="select * from survey_answer1 where survey_id='".$id."' and restaurant_id='".$admin_id."' ORDER BY answer_date DESC";
    ...
    function get_response2($id, $admin_id, $fromDateChart, $toDateChart){
        $sql="select * from survey_answer1 where survey_id='".$id."' and restaurant_id='".$admin_id."' and answer_date >= '".$fromDateChart."' and answer_date < ('".$toDateChart."' + INTERVAL 1 DAY) ORDER BY answer_date DESC";
        $data=  $this->db->query($sql,$id, $admin_id);
        return $data->result();
        //print_r($data->result()); exit();
    }
  ...
  public function response_view($device_id,$survey_id, $id){
      $sql="select * from survey_answer1 where sa_id='".$device_id."' and survey_id='".$survey_id."' and restaurant_id='".$id."' ORDER BY answer_date DESC";
  ...
  function get_response_for_waiter($survey_id2, $waiter_code, $id, $fromDate, $toDate){
      ...
      $sql = "select * from survey_answer1 where restaurant_id='".$id."' and survey_id='".$survey_id2."' and waiter_code='".$waiter_code."' and answer_date >= '".$fromDate."' and answer_date < ('".$toDate."' + INTERVAL 1 DAY) ORDER BY answer_date DESC";
  ...
  function get_response_for_waiter_all($survey_id2, $id, $fromDate, $toDate){
      $sql = "select * from survey_answer1 where restaurant_id='".$id."' and survey_id='".$survey_id2."' and answer_date >= '".$fromDate."' and answer_date < ('".$toDate."' + INTERVAL 1 DAY) ORDER BY answer_date DESC";
  ...
  function get_response_date($table,$id, $from, $current){
      $sql = "select * from survey_answer1 where survey_id='".$table."' and restaurant_id='".$id."' and answer_date >= '".$from."' and answer_date < ('".$current."' + INTERVAL 1 DAY) ORDER BY answer_date DESC";
...
//if select any device then filter to send data
    function get_device_data_responses($admin_id, $survey_id, $device_imei, $fromDate, $toDate){
        $sql="select * from survey_answer1 where restaurant_id='".$admin_id."' and survey_id='".$survey_id."' and device_imei='".$device_imei."' and answer_date >= '".$fromDate."' and answer_date < ('".$toDate."' + INTERVAL 1 DAY) ORDER BY answer_date DESC";
...
//if select all then imei device never find then all data view on dishplay
    function get_device_data_responses_noimei($admin_id, $survey_id, $fromDate, $toDate){
        $sql="select * from survey_answer1 where restaurant_id='".$admin_id."' and survey_id='".$survey_id."' and answer_date >= '".$fromDate."' and answer_date < ('".$toDate."' + INTERVAL 1 DAY) ORDER BY answer_date DESC";
